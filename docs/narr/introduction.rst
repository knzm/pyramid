...
似ていますが、例外が Pyramid 自体に「bubble up する」場合にのみ起動
...
見つからない場合に起こる例外や、あるいはユーザが何かをする許可を持たない
...
Pyramid は、「シングルトン」データ構造を正確に1つも持たないことをアプリ
ケーションに要求するような方法で書かれています。あるいは、別の言い方を
すれば Pyramid は「書き換え可能なグローバル変数」を構築することを一切
要求しません。また別の言い方をすれば、 Pyramid アプリケーションのインポー
トは「インポート時副作用」を持つ必要がありません。これは深淵なテーマの
ように聞こえますが、あなたがこれまでに Django の "settings.py" ファイル
を同じアプリケーションの複数のインストールに対してパラメータ化しようと
したり、あるフレームワークの fixture をあなたのユースケースに対して適切
に動作させるためにモンキーパッチを当てる必要があったり、システムをデプ
ロイするために非同期サーバーを使用しようとしたことがあれば、最終的に
...
ある1つのビューにディスパッチし、リクエストメソッドが POST だったら別の
ビューにディスパッチすることなどができます。「ビュー述語」として知られ
ているシステムはこれを実現します。リクエストメソッドの一致はビュー述語で
...
それは独力で矛盾する設定ステートメントを自動的に解決するでしょう:
「ローカルな」ステートメントは「ローカルでない」ものより優先されます。
これは、賢明なやり方で大規模なシステムをより小さなシステムの要素として
含めることを可能にします。
...
Pyramid は柔軟でプラグインできる認証と認可のシステムを含んでいます。
ユーザデータがどこに保存されるか、ユーザがデータにアクセスするのを許可
するためにどんなスキームを使用するかに関わらず、カスタムの認証と認可
コードをプラグインするためのあらかじめ定義された Pyramid プラグポイント
を使用することができます。後になってこれらのスキームを変更したければ、
コードのあちこちではなく、たった1ヶ所を変更するだけで済みます。さらに、
Pyramid にはすでに構築されて十分テストされている認証および認可スキームが
...
ができます。データが自然には木のようでない場合 (あるいは、データの木の
ような表現を作成したくない場合)、トラバーサルは非常に有用であるとは感じ
...
部門の階層の変更に自然に向いているサイトには非常に適しています。
...
ができる一種の内部 WSGI ミドルウェア的パイプラインがあります。 デバッグ
ツールバーは "tween" です。 ``pyramid_tm`` トランザクション管理もそうです。
tween は Pyramid 自身のコンテキストの中で実行されるので、いくつかの状況
...
を返すことが認められるかに燗する哲学から多くのことが作られています。
...
多くの情報があるからです。しかし、あなたがそのような美学を評価する
...
あ、しまった! もしカスタムなコンテントタイプを指定したかったら
どうしますか? カスタムなステータスコードは? 怖がることはありません。
...
それが最も一般に有用なので、 Pyramid はデフォルトで明示的に振る舞いますが、
フレームワークを局所的な審美的要望に適応させるためのフックを提供します。
...
Pyramid コンフィギュレータにディレクティブを加えることができます:
...
``instrumental`` ツールによる測定で 95% 以下の decision/condition
...
私たちは良いテストを行う regime を持たない他のプロジェクトで働いている間に、
...
ユーザを非難することに喜びを見い出すような他の人々を許容しません。
私たちは、そこを明るく、新規ユーザーフレンドリーに保とうとしています。
...
追加するには特定すぎるものです。どんな場合も、 Pyramid ドキュメンテーション
は包括的です。
